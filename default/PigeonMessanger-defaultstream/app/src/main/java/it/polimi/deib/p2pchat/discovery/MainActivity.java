...
import it.polimi.deib.p2pchat.discovery.utilities.CompressionUtils;
...
import static it.polimi.deib.p2pchat.discovery.Configuration.MESSAGERETURN;
...
public class MainActivity extends AppCompatActivity implements
    ...
    private final int RETURN_CODE = 364;
    public ChatManager chatManager2;
    private WifiP2pManager manager;
    private WifiP2pDnsSdServiceRequest serviceRequest;

    private BasicBroadcastReceiver basicReceiver = null;

    public static String getTAG() {
        return TAG;
    }

    public boolean isRetryChannel() {
        return retryChannel;
    }
    ...
    public void setDiscoveryStatus(boolean discoveryStatus) {
    ...
    public void setServiceRequest(WifiP2pDnsSdServiceRequest serviceRequest) {
    ...
    public void setSocketHandler(Thread socketHandler) {
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case Configuration.MESSAGE_READ:
                ...
                } else if (readMessage.contains(Configuration.MESSAGEEXCHANGE)) {

                    Log.d("MainActivity", "reading stream : " + readMessage);
                    String message = readMessage.replace(Configuration.MESSAGEEXCHANGE, "");
                    Intent intent = SerializeDeserialize.DeserializeIntent(message);
                    startActivityForResult(intent, RETURN_CODE);

                } else {//(readMessage.contains(Configuration.MESSAGERETURN)){


                    Log.d("MainActivity", "reading stream : " + readMessage);
                    startService(receiveIncomingMessage(readBuf));

                    /*readBuf = CompressionUtils.DecompressArray(readBuf);
                    readMessage = new String(readBuf);

                    Log.d("MainActivity", "reading stream : " + readMessage);
                    //String message = readMessage.replace(Configuration.MESSAGERETURN, "");
                    System.out.println("@@@ " + readMessage);

                    readMessage = readMessage.substring(readMessage.indexOf("{"), readMessage.length());
                    try {

                        Intent intent = SerializeDeserialize.DeserializeIntent(readMessage);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            ...
            /*case RETURN_CODE: {
                ...
                Log.d(TAG, "REQUEST REMOTE TRANSMISSION");

                Uri dataUri = data.getData();
                Cursor cursor = this.getContentResolver().query(dataUri, (String[])null, (String)null, (String[])null, (String)null);
                Intent sendIntent = CopyUtils.processDataForTransmission(cursor);

                if(data != null){
                    System.out.println(data.toString());
                    String json = SerializeDeserialize.SerializeIntent(sendIntent);
                    if(chatManager2 != null){
                        System.out.println("Sending information : " + json);
                        chatManager2.write(IntentSerialization.returnByteArrayFromString(json));
                    } else {
                        System.out.println("chatmanager2 is null");
                    }
                }
            ...
            case RETURN_CODE: {
                ...
                Log.d(TAG, "REQUEST STREAM TRANSMISSION");
                Intent intent = handleStreamIntent(data);
                startService(intent);
    ...
    private void manageAddressMessageReception(P2pDestinationDevice p2pDevice) {
        ...
        if (chatManager != null) {
            ...
            basicReceiver.setChatManager(chatManager);
...
//    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        basicReceiver = new BasicBroadcastReceiver();
        registerReceiver(basicReceiver, filter);

    }

    public void receiveStream(ByteBuffer data, String extension){

        System.out.println("received stream");

        byte[] processed = null;

        System.out.println("data byte array " + data.hasArray());

        if (data.hasArray()) {
            System.out.println("data byte array ");
            final byte[] array = data.array();
            System.out.println("data byte array " + array.length);
            final int arrayOffset = data.arrayOffset();
            processed = Arrays.copyOfRange(array, arrayOffset + data.position(),
                    arrayOffset + data.limit());
        }

        System.out.println("Received processed byte array " + processed.length);
        ...
        if(processed != null) {
            ...
            try {
                ...
                Log.d("MainActivity", "temp file path: " + temp.getAbsolutePath());
                FileOutputStream fileOutputStream = new FileOutputStream(temp);

                byte[] incomingData = CompressionUtils.DecompressArray(processed);

                Log.d("MainActivity", "done with decompressing");
                //byte[] incomingData = data.array();

                fileOutputStream.write(incomingData);
                fileOutputStream.close();

                try{

                    File from = new File(temp.getAbsolutePath());
                    System.out.print("playing " + from.getTotalSpace());
                    from.deleteOnExit();
    ...
    public void receiveStream(byte[] data, String extension){

        System.out.println("Received processed byte array " + data.length);
        ...
        if(data != null) {
            ...
            try {
                ...
                byte[] incomingData = CompressionUtils.DecompressArray(data);
